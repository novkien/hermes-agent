...
This is separate from an externally managed `himalaya` skill, which lets the agent manage email through terminal commands and requires the external `himalaya` CLI plus a Himalaya config file.
