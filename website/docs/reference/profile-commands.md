...
| `--no-skills` | Legacy compatibility option that writes `.no-bundled-skills`. This owner fork already seeds no bundled skills; use `skills.external_dirs` to define the profile's actual skill policy. It still refuses combinations with cloning options that copy profile-local skills. |
