...
_SKILL_REVIEW_PROMPT = (
    "Review the conversation above for durable, reusable corrections to "
    "existing skills. Inspect actively and patch conservatively: a high "
    "write count is not the objective, and 'Nothing to save.' is the correct "
    "result when the evidence or target is unclear.\n\n"
    "Target shape of the library: CLASS-LEVEL skills, each with a rich "
    "SKILL.md and a `references/` directory for session-specific detail. "
    "Do not expand that library from this autonomous review.\n\n"
    ...
    "skill that was in play, so it is usually the right one to extend.\n"
    "  2. UPDATE AN EXISTING UMBRELLA (via skills_list + skill_view). "
    "If no loaded skill fits but an existing class-level skill does, "
    "patch it. Add a subsection, a pitfall, or broaden a trigger.\n"
    "  3. If no existing skill clearly owns the learning, do not create a "
    "replacement or adjacent skill. Say 'Nothing to save.' and stop.\n\n"
    "STRICT MUTATION POLICY: use only skill_manage action=patch with "
    "old_string/new_string against an existing skill. Never create a skill, "
    "fully rewrite SKILL.md, add/remove a support file, or delete a skill. "
    "Those operations require an explicit foreground user request. You may "
    "patch skills regardless of whether they are local, external, bundled, "
    "hub-installed, pinned, or user-owned. The sole excluded family is "
    "agent2agent-* (including category-qualified names); never patch it.\n\n"
    "Read-before-write (ENFORCED — skill_manage refuses otherwise): "
    "before you patch an existing skill's SKILL.md, call "
    "skill_view(name) for that skill during this review. Before you "
    "patch a supporting file, call skill_view(name, file_path=...) for "
    ...
    "write must be based on what skill_view just returned. If a write "
    "is refused with a read-before-write "
    ...
    "Excluded skills: never patch a skill whose leaf name starts with "
    "agent2agent-. If that family is the only relevant target, say "
    ...
    "verified reusable technique, say 'Nothing to save.' and stop."
)
...
_COMBINED_REVIEW_PROMPT = (
    ...
    "**Skills**: durable corrections to an existing skill. Inspect actively "
    "and patch conservatively; a high write count is not the objective.\n\n"
    ...
    "It was in play, so it is usually the right place.\n"
    ...
    "agent2agent-* (including category-qualified names); never patch it.\n\n"
    "Read-before-write (ENFORCED — skill_manage refuses otherwise): "
    "before patching an existing skill's SKILL.md, call "
    "skill_view(name) during this review; before overwriting or "
    "patching a supporting file, call skill_view(name, file_path=...) "
    "for that exact file. Content quoted earlier in "
    "the transcript does NOT count — base the write on what "
    "skill_view just returned. On a read-before-write refusal: view the "
    ...
    "troubleshooting skill — never 'this tool does not work' as a "
    "standalone constraint.\n\n"
    "Act only when one of the two dimensions has durable, concrete signal. "
