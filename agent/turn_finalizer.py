...
def finalize_turn(
    ...
    *,
    ...
    # This is deliberately the last operation before return. Atomically close
    # the live steer window and drain its final payload only after every
    # fallible finalization hook has completed. A concurrent caller therefore
    # has exactly two outcomes: its text lands in this result, or it observes
    # the closed window and the surface queues it as a new turn. There is no
    # accepted-but-undeliverable gap, even if an earlier cleanup hook raises.
    _close_steer_window = getattr(agent, "_close_steer_window", None)
    if callable(_close_steer_window):
        _leftover_steer = _close_steer_window()
    else:
        # Lightweight test/dry-run agents may implement only the historical
        # drain helper. Production AIAgent always owns the atomic close path.
