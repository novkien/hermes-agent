...
11. Stage disposable work first, verify the intended output, then promote only verified material to its canonical destination.
12. Preserve original inputs unless overwrite is explicitly authorized. Never convert an edit request into destructive replacement by default.
13. Clean disposable temporary material after successful promotion. If residue must remain, report its exact path and retention reason.
14. Move uncertain untracked workspace material to dated `trash/` when reversible disposal is required. Use repository/project semantics for tracked files; never move tracked source into Hermes trash.
15. Do not change ignore files merely to hide files created by the current task. Add shared ignore rules only when they are a genuine project convention.
16. Multi-file consumer deliverables require a containing directory and a concise manifest or README unless the active workflow defines another package contract.
17. At completion, return exact retained paths, artifact classes, canonical project mutations, cleanup status, and any intentional residue. Never claim a file, upload, promotion, or cleanup without direct evidence.
18. If no authoritative permanent destination exists, keep the result in the task-scoped temporary namespace and report that unresolved destination instead of inventing one."""
