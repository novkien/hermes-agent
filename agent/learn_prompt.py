...
def build_learn_prompt(user_request: str) -> str:
    ...
    return (
        ...
        "2. Run the quality gate BEFORE saving:\n"
        "   2a. Overlap check — use `search_files(pattern='<keywords>', "
        "path='~/.hermes/skills', target='content')` to grep existing skills "
        "by keyword. Also check memory (`memory` tool or `MEMORY.md`).\n"
        "   2b. Append consideration — could the content fit an existing "
        "skill? If yes, name the target.\n"
        "   2c. Reusability check — is this a one-off fix or a pattern "
        "useful across 2+ future sessions?\n"
        "3. Form a holistic verdict:\n"
        "   - Save (unique, reusable, well-scoped) → write full draft, "
        "present to user, wait for approval → "
        "`skill_manage(action='create', ...)`.\n"
        "   - Improve then Save (valuable but vague) → refine → re-check "
        "→ if Save now, proceed as Save.\n"
        "   - Absorb into [X] (fits existing skill) → `skill_view(name='[X]')` "
        "→ show user diff → on approval: "
        "`skill_manage(action='patch', name='[X]', ...)`.\n"
        "   - Drop (trivial, redundant, one-off, too abstract) → show "
        "reasoning; no save needed.\n\n"
        "4. Follow the verdict. If Save: author ONE SKILL.md using the "
        "standards below. Pick a sensible category. If the procedure needs "
