...
class TestRunConversation:
    ...
    def test_provider_boundary_capture_preserves_full_tool_round_trip(
        self, agent, tmp_path
    ):
        ...
        agent._gateway_session_key = "agent:main:telegram:group:-100:42"
        tc = _mock_tool_call(
            name="web_search", arguments='{"query":"exact"}', call_id="call-1"
        )
        ...
        try:
            with (
                patch("run_agent.handle_function_call", return_value="exact result"),
                patch.object(agent, "_persist_session"),
                patch.object(agent, "_save_trajectory"),
                patch.object(agent, "_cleanup_task_resources"),
            ):
                result = agent.run_conversation("search exactly")

            records = session_db.get_llm_provider_requests(agent.session_id)
        finally:
            session_db.close()

        assert result["final_response"] == "Done"
        assert len(records) == 2
        assert [record["api_call_count"] for record in records] == [1, 2]
        assert all(
            record["session_key"] == "agent:main:telegram:group:-100:42"
            for record in records
        )
        second_messages = records[1]["payload"]["messages"]
        ...
        assert second_messages[2]["tool_calls"][0]["function"] == {
            "name": "web_search",
            "arguments": '{"query":"exact"}',
        }
        assert second_messages[3]["tool_call_id"] == "call-1"
        assert second_messages[3]["content"] == "exact result"

    def test_provider_boundary_capture_flattens_sdk_extra_body(
        self, agent, tmp_path
    ):
        from hermes_state import SessionDB
        ...
        try:
            agent._record_provider_request_payload(
                {
                    "model": "test-model",
                    "messages": [{"role": "user", "content": "hello"}],
                    "extra_body": {
        ...
        assert payload["provider"] == {"order": ["local"]}
