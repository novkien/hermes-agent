...
def test_kanban_guidance_prompt_size_bounded(monkeypatch, tmp_path):
    ...
    # Ceiling raised to 6_500 after the workspace-kind / deliverable-artifacts
    # expansion pushed the block past 5_500. Still guards unbounded growth.
    assert 1_500 < len(KANBAN_GUIDANCE) < 6_500, (
        f"KANBAN_GUIDANCE is {len(KANBAN_GUIDANCE)} chars — too short (missing?) or too long"
    )


def test_kanban_guidance_suppressed_for_denied_thread(monkeypatch, tmp_path):
    """A session whose thread_id is in kanban.disable_guidance_threads must
    NOT receive KANBAN_GUIDANCE, even if kanban_show is in the tool surface.

    Guards CEO coordinator topics that keep kanban_* tools for routing
    but must not be framed as Workers by the lifecycle guidance.
    """
    # Worker env NOT set (gateway chat session, not dispatcher spawn).
    monkeypatch.delenv("HERMES_KANBAN_TASK", raising=False)
    home = tmp_path / ".hermes"
    home.mkdir()
    monkeypatch.setenv("HERMES_HOME", str(home))
    ...
    # Write a config that denies thread 32857.
    ...
    a = AIAgent(
        ...
        skip_context_files=True,
        skip_memory=True,
        thread_id="32857",  # CEO topic — denylisted
    )
    prompt = a._build_system_prompt()
...
def test_kanban_guidance_kept_for_non_denied_thread(monkeypatch, tmp_path):
    """A session with kanban toolset whose thread_id is NOT in the denylist
    still receives KANBAN_GUIDANCE (Managers/orchestrators keep it)."""
    monkeypatch.delenv("HERMES_KANBAN_TASK", raising=False)
    home = tmp_path / ".hermes"
    home.mkdir()
    monkeypatch.setenv("HERMES_HOME", str(home))
    ...
    a = AIAgent(
        ...
        skip_context_files=True,
        skip_memory=True,
        thread_id="36319",  # Coder Manager — NOT denylisted
    )
    prompt = a._build_system_prompt()
    assert "Kanban task execution protocol" in prompt
    assert "kanban_show()" in prompt
...
def test_kanban_guidance_worker_overrides_denylist(monkeypatch, tmp_path):
    """HERMES_KANBAN_TASK set always wins over the thread denylist.

    A dispatcher-spawned worker in a denied thread still gets the
    lifecycle guidance (it is a real worker, not a coordinator).
    """
    ...
    a = AIAgent(
        ...
        skip_context_files=True,
        skip_memory=True,
        thread_id="32857",  # denied, but worker env overrides
    )
    prompt = a._build_system_prompt()
    assert "Kanban task execution protocol" in prompt
