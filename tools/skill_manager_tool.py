...
SKILL_MANAGE_SCHEMA = {
    ...
    "description": (
        ...
        f"first {SKILL_PROMPT_DESC_LIMIT - 3} chars "
        "('Use when <trigger>. <one-line behavior>.'), numbered steps with "
        "exact commands, pitfalls, verification (see skill_view() for "
        "format). Confirm with the user before create/delete. Pinned skills "
        "can still be patched, but deletion is refused until the user runs "
        ...
        "properties": {
            ...
            "name": {
                ...
                "description": (
                    ...
                    "Must match an existing skill for patch/delete/write_file/remove_file."
                ),
            },
            "content": {
                "type": "string",
            ...
            "file_path": {
                ...
                "description": (
                    ...
                    "For 'write_file'/'remove_file': required, must be under "
            ...
            # The handler also accepts ``absorbed_into`` on delete and the
            # internal atomic ``operations`` batch shape. Neither is public.
        },
