...
export function workerContextNodes(link, { compact = false, showStatus = true } = {}) {
  if (!link) return [];
  ...
  if (showStatus) result.push(el('span', { class: `chip chip-${link.status === 'running' ? 'live' : link.status === 'blocked' ? 'warn' : link.status === 'done' ? 'ok' : 'idle'}`, text: link.status }));
