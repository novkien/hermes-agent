...
assert.match(tab, /activeRepos/);
assert.match(tab, /Syncing…/);
assert.match(tab, /remain unpushed/);
...
assert.match(tab, /Historical failure:/);
assert.match(tab, /return repo\?\.error \|\| null/);
assert.doesNotMatch(tab, /return repo\?\.error \|\| repo\?\.last_operation\?\.error/);
