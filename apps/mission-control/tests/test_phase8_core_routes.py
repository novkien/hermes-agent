...
def mutation_request(
    method: str, path: str, body: dict, query: str = "profile=alpha"
) -> Request:
...
async def main_async() -> None:
    ...
    class Dashboard:
        async def request(self, method, path, **kwargs):
            ...
            assert kwargs["json_body"] == {
                "updates": {"name": "nightly", "enabled": False}
            }
            return (
                200,
                {"job": {"id": "j1", "name": "nightly", "state": "scheduled"}},
                {},
            )
    ...
    class Bus:
        async def safe_publish(
            self, event_type, source_id, entity_type, entity_id, payload, **kwargs
        ):
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        router._guard_mutation = lambda _request: (
            order.append("guard") or {"id": "owner"}
        )
