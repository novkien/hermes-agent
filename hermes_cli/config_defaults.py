...
DEFAULT_CONFIG = {
    ...
    "skills": {
        # Controls selected descriptions in the always-on <available_skills>
        # index. Skills absent from both lists stay fully visible. Skill
        # loading and SKILL.md contents are unaffected.
        "mode": {
            "prune": [],       # skill names whose descriptions cap at 60 chars
            "invisible": [],   # skill names rendered without descriptions
        },
    ...
    "telegram": {
        ...
        "extra": {
            "rich_messages": False,     # Bot API 10.1 rich messages (tables/task lists/horizontal rules/details/math) render natively; set True to opt in. Default stays legacy MarkdownV2 because rich messages can be hard to copy as plain text in Telegram clients.
