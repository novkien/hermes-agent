...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        # Active-session guard: skip gateway restart if sessions are active.
        # Restarting gateways while sessions are active kills in-flight
        # conversations (Issue #96).  The check is best-effort — if the
        # session registry is unavailable, proceed with restart.
        _skip_gateway_restart = False
        try:
            from hermes_cli.active_sessions import active_session_registry_snapshot

            _active = active_session_registry_snapshot()
            if _active:
                print()
                print(
                    f"⚠ {len(_active)} active Hermes session(s) detected"
                )
                print(
                ...
                _skip_gateway_restart = True
        except Exception:
            # Non-fatal — proceed with restart if the check itself fails.
            pass

        if not _skip_gateway_restart:
